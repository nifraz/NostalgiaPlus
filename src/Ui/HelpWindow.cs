    ...
    public sealed class HelpWindow : Form
    {
        ...
        private void CollectOverview()
        {
            ...
            Add(S, "The centre deck",
                ...
                + "It is in three parts. The goniometer sits on the middle of the deck - which is the "
                + "middle of the screen, and the axis the whole display is mirrored about - and the two "
                + "halves it leaves are equal by construction. What is playing goes in the left half: "
                + "the artwork, then the title, composer, artists, album and year. What the sound is "
                + "doing goes in the right: the transport, the phase meters, and the readouts.\n\n"
                + "How many metadata lines you get depends on the deck's height. At 150px each field is "
                + "named on its own line; at 120 they pair up as composer and artists, then album and "
                + "year; below that the title stands alone. Composer appears only if MusicBee reports a "
                + "field by that name - the tag id is looked up at startup rather than assumed, because "
                + "a wrong id would return some other tag and say nothing about it.\n\n"
                + "The transport, the seek bar and the clock share one row, and that bar sits in the "
                + "same column as the correlation and balance bars below it, so the three line up down "
                + "both edges. If the block is too narrow to hold controls, bar and clock side by side, "
                + "the seek bar takes a row of its own instead.\n\n"
                ...
                + "Every item has its own switch under View - Centre deck contents, and the deck only "
                + "gets the gap the two graph strips leave. It holds what fits and sheds the rest, "
                + "readout columns first and from the right, so brightness and tempo go before either "
                + "LUFS figure does. A single transport row is asked for ahead of the last readout "
                + "columns, but never at the price of all of them.\n\n"
                + "At a 45% graph on a 1920-wide screen that gap is about 850px, which is enough for "
                + "six of the nine readouts alongside everything else. Widening the graph strips buys "
                + "more; so does switching off the readouts you do not watch. Raising the deck height "
                + "past 120px gives the grid a third row, which is how nine readouts fit in three "
                + "columns instead of five - beyond that, height buys lines of metadata rather than "
                + "readouts. Press O to hide the deck altogether.", 1);
